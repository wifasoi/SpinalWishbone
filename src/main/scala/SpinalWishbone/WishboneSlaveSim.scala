...
case class WishboneSlaveFactory(wishbone: Wishbone, clockdomain: ClockDomain) extends WishboneSlave{
  fork{
    while(true){
 ...
 */
case class WishbonePipelinedSlaveFactory(wishbone: Wishbone, clockdomain: ClockDomain) extends WishboneSlave{
  fork{
    while(true){
...
class WishboneSlaveTest extends Component {
  val io = new Bundle{
    val bus = master(Wishbone(WishboneConfig(8,8,useCYC = true)))
    val start = in Bool
  }
...
object SlaveTest{
  def main(args: Array[String]): Unit = {
    SimConfig(rtl = new WishboneSlaveTest).withWave.doManagedSim{ dut =>
      ...
      sleep(1000)
      dut.io.start #= true
      val wbs = new WishboneSlaveFactory(dut.io.bus, dut.clockDomain)
